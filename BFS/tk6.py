...
class NetworkCrawlerGUI:
    ...
    def create_control_frame(self):
        ...
        # Replace Add Row with Import CSV
        ttk.Button(self.control_frame, text="Import CSV", command=self.import_csv).pack(side=tk.LEFT, padx=5,
                                                                                        expand=True)
    ...
    def generate_graph(self):
        ...
        # Identify node types
        spine_nodes = [n for n in self.G.nodes() if 'spine' in n.lower()]
        leaf_nodes = [n for n in self.G.nodes() if 'leaf' in n.lower()]
        host_nodes = [n for n in self.G.nodes() if 'host' in n.lower()]
        other_nodes = [n for n in self.G.nodes() if n not in spine_nodes + leaf_nodes + host_nodes]

        # Calculate vertical levels
        max_width = max(len(spine_nodes), len(leaf_nodes), len(host_nodes), len(other_nodes))
        self.scale = 2.0 / max_width if max_width > 0 else 1.0

        self.pos = {}

        # Position nodes by type and level
        def position_nodes(nodes, level, default_y=0):
            width = len(nodes)
            if width == 0:
                return
            ...
            for i, node in enumerate(sorted(nodes)):
                x = (i + 1) * spacing - 1.0  # Center around 0
                self.pos[node] = (x * self.scale, level * self.scale)

        # Position each layer
        position_nodes(spine_nodes, 2.0)  # Top layer
        position_nodes(leaf_nodes, 1.0)  # Middle layer
        position_nodes(host_nodes, 0.0)  # Bottom layer

        # Position any other nodes in an additional layer if needed
        if other_nodes:
            position_nodes(other_nodes, -1.0)  # Extra layer below hosts
