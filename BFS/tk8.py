...
class NetworkCrawlerGUI:
    ...
    def create_control_frame(self):
        # Create frame for buttons and layout selector
        self.control_frame = ttk.Frame(self.right_container, padding="5")
        self.control_frame.grid(row=2, column=0, sticky="nsew")

        # Create layout selection frame
        layout_frame = ttk.Frame(self.control_frame)
        layout_frame.pack(side=tk.TOP, fill=tk.X, pady=(0, 5))

        # Add layout label and combobox
        ttk.Label(layout_frame, text="Layout:").pack(side=tk.LEFT, padx=5)
        self.layout_var = tk.StringVar(value="Hierarchical")
        self.layout_combo = ttk.Combobox(layout_frame,
